...
export function parseBlock(body: string): { commands: string[]; expect: Expect; steps: Step[] } {
  ...
  const finalizeStep = () => {
    if (currentCmd !== null) {
      ...
      while (expOut.length > 0 && expOut[expOut.length - 1] === '') expOut.pop()
      while (expErr.length > 0 && expErr[expErr.length - 1] === '') expErr.pop()
      steps.push({
        cmd: currentCmd,
...
export function matchLines(
  ...
  while (i < expected.length) {
    ...
    if (e.trim() === '[...]' || e.trim() === '...') {
      ...
      return { ok: false, msg: 'ellipsis could not align with subsequent expected lines' }
    }

    if (j >= actual.length)
      return { ok: false, msg: `missing actual line for expected: ${JSON.stringify(e)}` }
  ...
  if (j !== actual.length)
    return { ok: false, msg: `extra actual lines starting at ${JSON.stringify(actual[j])}` }
