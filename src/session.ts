...
export class TestSession {
  ...
  async loadState(): Promise<void> {
    ...
    if (!existsSync(this.cwdFile)) writeFileSync(this.cwdFile, this.testTempDir, 'utf8')
  ...
  async runCommand(
    ...
    let stdout = splitNorm(result.stdout.toString())
    let stderr = splitNorm(result.stderr.toString())
  ...
  private buildCommandScript(command: string): string {
    ...
    if (afterEach) parts.push(`[ -f "${afterEach}" ] && source "${afterEach}" 2>/dev/null || true`)
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      if (existsSync(this.testTempDir)) rmSync(this.testTempDir, { recursive: true, force: true })
    } catch {
      // Ignore cleanup errors
    }
  }
