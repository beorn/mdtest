...
interface TestStructure {
  headings: Array<{
    ...
    slug: string
    steps: Array<{ cmd: string; expected: any }>
    index: number // Original index in document
  }>
}

function buildTestStructure(codeBlocks: any[], headings: any[], _filePath: string): TestStructure {
  ...
  for (let i = 0; i < codeBlocks.length; i++) {
    ...
    if (!['console', 'sh', 'bash'].includes(block.lang)) continue
...
function buildHeadingPath(heading: any, allHeadings: any[]): string[] {
  if (!heading) return ['(no heading)']

  // Count H1 headings (depth 1)
  const h1Count = allHeadings.filter((h: any) => h.depth === 1).length
...
function registerBunTests(filePath: string, structure: TestStructure, codeBlocks: any[]): void {
  ...
  describe.serial(filePath, () => {
    ...
    const pathGroups = new Map<
      string,
      Array<{ steps: Array<{ cmd: string; expected: any }>; index: number }>
    >()
    ...
    for (const heading of structure.headings) {
      ...
      pathGroups.get(key)!.push({ steps: heading.steps, index: heading.index })
    }
...
function registerNestedTests(
  path: string[],
  items: Array<{ steps: Array<{ cmd: string; expected: any }>; index: number }>,
  session: TestSession,
  depth = 0
): void {
    ...
    for (const item of items) {
      for (const step of item.steps) {
        ...
        test.serial(`$ ${testName}`, async () => {
          ...
          if (!result.passed) throw result.diff || 'Command output mismatch'
        })
      }
    }
    return
...
export async function bunShell(cmd: string[], opts?: ShellOptions): Promise<ShellResult> {
  ...
  const processPromise = (async () => {
    ...
    if (process.env.DEBUG_MDTEST) {
      ...
      console.error(`[bunShell]   stdout preview: ${JSON.stringify(stdout.slice(0, 100))}`)
    }
